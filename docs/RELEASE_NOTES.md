...
- Shared state for a widget type the session has not registered is written back on export instead of dropped, so saving from such a session no longer loses that widget family's configuration
...
- Custom Error Views page: three cells whose widget types are missing, each answered differently by `UNKNOWN_WIDGET_RESOLVER`, with buttons to load and unload each type and watch its cell heal in place

### Refactoring

- The navigation menu uses `routerLink` instead of click handlers
